"""
MODULE 3: PCOA Structure Discovery - FIXED INHIBITION LOGIC

CRITICAL FIX: Inhibitions detected by checking equation symbols, not lineage
"""
...
class PCOADiscovery:
    ...
    def discover_structure(self, equilibrium_equations, debug=False):
        ...
        6. Add inhibitions via symbol detection
        7. Convert to DAG
        ...
        # FIXED: Add inhibitions using symbol detection
        cluster_graph = self._add_inhibitions(cluster_graph, selected_eqs, debug=debug)
    ...
    def _add_inhibitions(self, cluster_graph, selected_equations, debug=False):
        """
        Add inhibition nodes by DETECTING WHERE THEY APPEAR in equations
        
        FIXED: Checks equation symbols directly, not lineage
        """
        ...
        for var, inhib_name in self.inhib.items():
            if debug:
                print(f"\n  Detecting {inhib_name} in equations:")
            
            added_count = 0
            for eq_name, eq_expr in selected_equations.items():
                # FIXED: Check if inhibition appears (string-based for robustness)
                sym_names = {str(s) for s in eq_expr.free_symbols}
                if inhib_name in sym_names and eq_name in eq_to_cluster:
                    target_cluster = eq_to_cluster[eq_name]
                    ...
                    if debug:
                        print(f"    {eq_name}: Contains {inhib_name} → Added to cluster {target_cluster}")
            
            if debug and added_count == 0:
                print(f"    ⚠️  {inhib_name} NOT found in any selected equations!")
