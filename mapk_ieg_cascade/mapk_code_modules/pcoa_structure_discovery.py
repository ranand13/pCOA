"""
PCOA Structure Discovery - WITH MATCHING INFORMATION

Returns:
- structure: {var: [parents]} for BOTH original AND R variables
- topo_order: topological order including R variables
- var_to_equation: {var: equation_name} for detecting deterministic vars
- selected_equations: {eq_name: sympy_expr}
"""
...
class PCOADiscovery:
    ...
    def discover_structure(self, equilibrium_equations, debug=False):
        ...
        6. Extract variable→equation matching
        7. Add inhibitions and convert to DAG
        ...
        Returns:
            structure: {var: [parent_list]} - includes R variables!
            topo_order: [var1, var2, ...] - includes R variables!
            var_to_equation: {var: equation_name}
            selected_equations: {eq_name: sympy_expr}
        """
        ...
        # Separate R equations before selection
        ...
        # Select for ORIGINAL variables only
        ...
        # Extract variable→equation matching BEFORE adding inhibitions
        var_to_equation = self._extract_matching(selected_eqs, transformed_vars, debug)
        
        if debug:
            print(f"\n[Variable→Equation Matching]")
            for var in list(self.vars) + [v for v in transformed_vars.keys() if v.startswith('R_')]:
                eq = var_to_equation.get(var, 'UNMATCHED')
                is_R = eq.startswith('f_R_') if eq != 'UNMATCHED' else False
                marker = ' [DETERMINISTIC]' if is_R else ''
                if var in var_to_equation:
                    print(f"  {var} ← {eq}{marker}")
        ...
        # Convert to variable DAG - KEEP R VARIABLES!
        dag = self._to_variable_dag_with_R(cluster_graph, transformed_vars)
        ...
        # Build structure for BOTH original AND R variables
        structure = {}
        for var in self.vars:
            structure[var] = list(dag.predecessors(var)) if var in dag else []
        
        # Add R variables to structure
        for var in dag.nodes():
            if var.startswith('R_') and var not in structure:
                structure[var] = list(dag.predecessors(var)) if var in dag else []
        
        return structure, topo, var_to_equation, selected_eqs
    
    def _extract_matching(self, equations, variables, debug=False):
        """
        Extract variable→equation matching from bipartite graph
        
        Returns:
            {var_name: equation_name} for ALL variables (original + R)
        """
        # Build bipartite graph
        B = nx.Graph()
        
        # Add variable nodes (both original and R variables)
        for var_name in variables.keys():
            B.add_node(f"v_{var_name}", bipartite=0)
        
        # Add equation nodes
        for eq_name in equations.keys():
            B.add_node(eq_name, bipartite=1)
        
        # Add edges
        for eq_name, expr in equations.items():
            for var_name, var_sym in variables.items():
                if var_sym in expr.free_symbols:
                    B.add_edge(f"v_{var_name}", eq_name)
        
        # Find maximum matching
        left = {n for n, d in B.nodes(data=True) if d.get('bipartite') == 0}
        M = nx.bipartite.maximum_matching(B, top_nodes=left)
        
        # Extract variable→equation mapping (both original AND R variables)
        var_to_eq = {}
        
        for var_name in variables.keys():
            var_node = f"v_{var_name}"
            if var_node in M:
                var_to_eq[var_name] = M[var_node]
        
        return var_to_eq
    
    def _to_variable_dag_with_R(self, cluster_graph, transformed_vars):
        """
        Convert cluster graph to DAG
        
        CRITICAL: Keeps R variables in the DAG!
        
        Returns DAG with:
        - Original variables (for final output)
        - R variables (needed for deterministic computations)
        - Exogenous variables (k_in, inhibitions)
        """
        dag = nx.DiGraph()
        cluster_contents = {}
        ...
        for cluster, data in cluster_graph.nodes(data=True):
            # Keep original variables AND R variables
            vars_in = [v for v in data.get('variables', []) 
                      if v in original_var_names or v.startswith('R_')]
            ...
            if vars_in or exo_in:
                ...
                for node in cluster_contents[cluster]:
                    if node in exogenous_names:
                        node_type = 'exogenous'
                    elif node.startswith('R_'):
                        node_type = 'R_variable'
                    else:
